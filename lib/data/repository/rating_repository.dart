...
import 'package:cabby/domain/models/rating_model.dart';
...
class RatingRepository {
  ...
  Future<Rating> submitRating(RatingRequest request) async {
    final response = await _apiProvider.post('/ratings', data: request.toJson());
  ...
  Future<List<Rating>> getRiderRatings(String riderId) async {
    final response = await _apiProvider.get('/ratings/rider/$riderId');
    return (response.data as List).map((rating) => Rating.fromJson(rating)).toList();
  }

  Future<RatingStats> getRiderRatingStats(String riderId) async {
    final response = await _apiProvider.get('/ratings/rider/$riderId/stats');
  ...
  Future<void> updateRating(String ratingId, RatingRequest request) async {
    await _apiProvider.put('/ratings/$ratingId', data: request.toJson());
  }
