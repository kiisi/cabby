...
/// This class adds an overlay and builds [_MarkerHelper] on that overlay.
...
///
/// These native libraries accept BitmapDescriptor for markers, which means that for custom markers
/// you need to draw the view to a bitmap and then send that to BitmapDescriptor.
///
/// Because of that, Flutter also cannot accept a Widget for a marker, but you need to draw it to a bitmap and
/// that's what this widget does:
///
/// 1) It draws the marker widget to the tree.
/// 2) After being painted, it accesses the repaint boundary with a global key and converts it to uInt8List.
/// 3) Returns a set of Uint8List (bitmaps) through a callback.
